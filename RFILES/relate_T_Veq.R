...
makeplots <- function(mem,label=NULL) {
  ...
  p <- ggplot(data=equilibrium_volume, aes(x=Tgrl, y=Veq, shape=SIClabel)) +
    geom_point(colour="blue", size=3) + 
    geom_line(data=Veq.model, aes(x=Tgrl, y=Veq, colour=fit)) +
...
# PARAMETERS LINEAR FIT and FIGURE MAIN ARTICLE
mem=8  
label = NULL
SIClabel <- ifelse(is.null(label),"SICOPOLIS",paste("SICOPOLIS \n",label,sep=""))
#EMlabel <- ifelse(is.null(label),"Emulator",paste("Emulator of \n",label,sep="")) 
makeplots(mem=mem,label=label) # defines necessary variables 'globally'

# write parameters a and b
write.table(data.frame(a=a,b=b),"data/params.Veq.txt",row.names=F)

# make figure
p <- ggplot(data=equilibrium_volume, aes(x=Tgrl, y=Veq, shape=SIClabel)) +
  geom_point(colour="blue", size=3) + 
  geom_line(data=Veq.model, aes(x=Tgrl, y=Veq, colour=fit)) +
  labs(x="Temperature with respect to 1976-2005 [deg C]",
       y="Equilibrium ice volume [m SLR]",
       shape="Ice sheet model",
       color="Emulator") +
  theme(legend.position=c(0.8,0.7))
pdf("figures/equilibrium_GIS.pdf", paper="special", width=5, height=5/1.618); print(p); dev.off()  
...
p27 <-  p27 +
  labs(shape = "Ice sheet model",
               color="Emulator") +
  theme(legend.position=c(0.7,0.6),
        legend.key.height = unit(0.5,"lines"))

pdf("figures/supplement_fig_relation_T_Veq.pdf", paper="special", width=10, height=10*1.3)
grid.arrange(
  arrangeGrob(p1,
              p2,
              p3,
  ...
  left = textGrob("Equilibrium ice volume [m SLR]",               rot = 90, vjust = 0.5, hjust = 0.0,
                  gp = gpar(cex = 1.)),
  bottom = textGrob(expression(paste("Temperature anomaly [",degree,"C]")),, rot = 0,  vjust = 0,   hjust = 0.5,
                 gp = gpar(cex = 1.))
)
dev.off()
