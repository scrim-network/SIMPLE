...
# 'Delayed-linear' is a semi-empirical model (sem) to estimate volumetric change
...
#
# variables needed
# params: numeric vector containing 'a', 'b' and 'tau'
...
sem_traject = function(params, V0, Tt) {  
  ...
  for (i in 2:nt) {
    Vt[i] <- max( 0,  Vt[i-1] + -(Vt[i-1]-Veq[i]) / tau )
  } # end for (i in 2: nt)
